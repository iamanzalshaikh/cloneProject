...
export const addListing = async (req, res) => {
  try {
    ...
    // Safe path checks
    const img1Path = req?.files?.image1?.[0]?.path;
    const img2Path = req?.files?.image2?.[0]?.path;
    const img3Path = req?.files?.image3?.[0]?.path;

    if (!img1Path || !img2Path || !img3Path) {
      return res.status(400).json({ message: "Missing image paths" });
    }

    console.log("📁 Uploading images to Cloudinary...");
    let image1 = await uploadOnCloudinary(img1Path);
    let image2 = await uploadOnCloudinary(img2Path);
    let image3 = await uploadOnCloudinary(img3Path);
